<x-admin-layout>
    <div class="max-w-4xl mx-auto py-10 px-6 bg-white shadow-lg rounded-xl">
        ...
        <form action="{{ route('admin.organizations.update', $organization->id) }}" method="POST"
            ...
            <div>
                <button type="submit"
                    class="bg-green-600 text-white px-6 py-2 rounded-lg shadow hover:bg-green-700 transition font-medium">
                    Update Organization
                </button>
            </div>
        </form>

        {{-- Existing Images Remove Section (separate forms, outside main form) --}}
        @if ($organization->images && count($organization->images))
            <div class="mt-8">
                <label class="block text-gray-700 font-medium mb-2">Existing Images</label>
                <div class="flex flex-wrap gap-4">
                    @foreach ($organization->image_urls as $index => $img)
                        <div class="text-center">
                            <img src="{{ $img }}" alt="Org Image"
                                class="w-24 h-24 object-cover rounded-lg border mb-2">
                            <form action="{{ route('admin.organizations.removeImage', [$organization->id, $index]) }}"
                                method="POST">
                                @csrf
                                @method('DELETE')
                                <button type="submit"
                                    class="w-full bg-red-100 text-red-700 hover:bg-red-200 px-3 py-1 rounded-md text-sm transition">
                                    Remove
                                </button>
                            </form>
                        </div>
