<x-admin-layout>
    <div class="max-w-3xl mx-auto py-10">
        <h2 class="text-2xl font-bold mb-6">Emergency Leave Request</h2>
        ...
        <form action="{{ route('vacations.emergency.store') }}" method="POST" enctype="multipart/form-data"
            ...
            <div class="mb-4">
                <label for="replacement_user_id" class="block font-medium mb-1">Replacement Employee</label>
                <select name="replacement_user_id" id="replacement_user_id" class="w-full border rounded p-2">
                    <option value="">Select Employee</option>
                    @foreach ($employees as $emp)
                        <option value="{{ $emp->id }}"
                            {{ old('replacement_user_id') == $emp->id ? 'selected' : '' }}>
                            {{ $emp->name }}
                        </option>
                    @endforeach
                </select>
